...
def score_listing(job_data, profile):
    ...
    # Geo gate: job location must mention a geo keyword OR be remote/hybrid
    remote_terms = ['remote', 'anywhere', 'hybrid', 'work from home']
    geo_in_location = any(kw.lower() in job_location for kw in profile.get('geo_keywords', []))
    is_remote = any(term in job_location for term in remote_terms)

    if profile.get('require_geo_match', False):
        if not geo_in_location and not is_remote:
            return None
    ...
    if geo_in_location or is_remote:
        matched.append('GEO_MATCH')
